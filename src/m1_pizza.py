...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(12.5, 5.0),  # All numbers are approximate.
                rg.Point(11.2, 7.2),
                rg.Point(8.8, 7.2),
                rg.Point(7.5, 5.0),
                rg.Point(8.7, 2.8),
                rg.Point(11.2, 2.8)]
    circle = rg.Circle(rg.Point(10, 5), 2.5)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 3:  4 blue dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(120, 120), 20)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(210, 210), 80)
    circle.outline_thickness = 5
    polygon(window, circle, 9, 'green', 5)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(1, len(points)):
        ...
        line.thickness = thickness
        line.attach_to(window)
    lastline = rg.Line(points[0], points[len(points) - 1])
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 10, 4, 'black', 1)
    window.close_on_mouse_click()
